...
async def part_to_openai_content(
  ...
  if part.text:
    ...
    return {"type": "text", "text": f"Function response: {part.function_response.response}"}
    ...
    if openai_instance:
      ...
      if part.inline_data:
        ...
        return {"type": "text", "text": f"[FILE REFERENCE: {part.file_data.display_name or 'unnamed'}]"}
  elif part.executable_code:
    return {"type": "text", "text": f"```python\n{part.executable_code.code}\n```"}
  elif part.code_execution_result:
    return {"type": "text", "text": f"Execution Result:\n```\n{part.code_execution_result.output}\n```"}
  else:
  ...
  for part in content.parts or []:
    if part.function_call:
      ...
      message_content.append({
          "type": "text",
          "text": json.dumps(part.function_response.response) if isinstance(part.function_response.response, dict) else str(part.function_response.response)
      })
    else:
...
class OpenAI(BaseLlm):
  ...
  async def _upload_file_to_openai(self, file_data: bytes, mime_type: str, display_name: Optional[str] = None) -> str:
    """Upload a file to OpenAI's Files API and return the file ID."""
    if not self.use_files_api:
      raise ValueError("Files API is disabled. Set use_files_api=True to enable file uploads.")
    ...
    try:
      ...
      logger.info(f"Uploaded file to OpenAI: {uploaded_file.id} ({display_name or 'unnamed'})")
  ...
  async def _handle_file_data(self, part: types.Part) -> Union[Dict[str, Any], str]:
    ...
    if part.inline_data:
      ...
      if self.use_files_api and mime_type in ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"]:
        # Upload documents to Files API
        try:
          file_id = await self._upload_file_to_openai(data, mime_type, display_name)
          return {
              "type": "text",
              "text": f"[File uploaded to OpenAI: {display_name or 'unnamed file'} (ID: {file_id})]"
          }
        except Exception as e:
          logger.warning(f"Failed to upload file to OpenAI Files API: {e}. Falling back to base64 encoding.")
