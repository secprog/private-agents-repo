...
Use proper mxGraph XML format with accurate positioning and appropriate styles."""
    ...
    async def iterative_analysis(
        self, user_id: str, session_id: str, filename: str, max_iterations: int = 3
    ) -> str:
        """Perform iterative analysis with quality improvement"""
        try:
            logger.info(
                f"Starting iterative analysis for session_id={session_id}, filename={filename}, max_iterations={max_iterations}"
            )
            ...
            while iteration <= max_iterations:
                ...
                logger.info(f"Iteration {iteration} quality score: {quality_score}")
            ...
            logger.error(f"Failed iterative analysis: {e}")
            raise
