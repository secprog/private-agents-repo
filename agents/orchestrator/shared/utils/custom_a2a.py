...
def create_a2a_server_with_shared_session(
    ...
    **kwargs
    ...
    This function creates an A2A server that uses the DatabaseSessionService
    instead of the default InMemorySessionService, ensuring session consistency
    across all agents through the shared PostgreSQL database.
    """
    ...
    def create_runner() -> Runner:
        """Create a runner with database session service."""
        return Runner(
            app_name=agent.name or "adk_agent",
            agent=agent,
            # Use session services instead of InMemory
    ...
    logger.info("✅ Created A2A server with proper session service")
    return app
