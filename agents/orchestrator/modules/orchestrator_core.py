...
class OrchestratorCore:
    ...
    def discover_agents_with_retry(self, max_retries: int = 5, retry_delay: int = 5):
        ...
        for attempt in range(max_retries):
            ...
            if final_count == len(self.discovery_endpoints):
                ...
                logger.info(f"📈 Discovered {final_count - initial_count} new agents, {final_count}/{len(self.discovery_endpoints)} total")
            
            if attempt < max_retries - 1:
                logger.info(f"⏳ Waiting {retry_delay}s before retry (some agents may still be starting up)")
        ...
        if discovered_count < total_endpoints:
            logger.warning(f"⚠️ Only discovered {discovered_count}/{total_endpoints} agents after {max_retries} attempts")
    ...
    def _create_sub_agents_from_registry(self) -> List[RemoteA2aAgent]:
        ...
        for agent_id, agent_info in self.agent_registry.items():
            try:
                ...
                valid_name = agent_id.replace("-", "_")  # Convert hyphens to underscores
                remote_agent = RemoteA2aAgent(
                    name=valid_name,
                    description=agent_info["description"],  # Use description from registry
    ...
    def _create_workflow_agent(self):
        ...
        try:
            ...
            self.workflow_agent.instruction = "Do nothing, just say: I am discovering available sub-agents..."
    ...
    def discover_agents(self):
        ...
        for endpoint in self.discovery_endpoints:
            try:
                ...
                try:
                    response = requests.get(agent_card_url, verify=verify_ssl, timeout=10.0)
                    if response.status_code == 200:
                        ...
                        self.agent_registry[agent_id] = {
                            ...
                            "description": agent_card.get("description", f"Agent {agent_id}"),
                        }
                        ...
                        if agent_id in self.agent_registry:
                            logger.info(f"❌ Agent {agent_id} no longer available, removing from registry")
        ...
        skills = [
            {
                ...
                "tags": ["llm", "orchestration", "routing", "delegation", "coordination"]
            }
        ]
        ...
        for agent_id, agent_info in self.agent_registry.items():
            ...
            if agent_skills:
                ...
                for skill in agent_skills:
                    ...
                    skill_copy["tags"] = skill_copy.get("tags", []) + [f"sub_agent:{agent_id}"]
